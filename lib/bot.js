...
const classifySexualSticker = async (media, msg) => {
    const apiKey = config.openaiApiKey || process.env.OPENAI_API_KEY;
    ...
    try {
        ...
        if (!looksLikeMediaBuffer(buffer, 'stickerMessage') && msg?.key?.id) {
            try {
                buffer = await downloadMediaMessage({
                    key: msg.key,
                    message: { stickerMessage: media }
        ...
        const response = await axios.post(`${apiBase}/chat/completions`, {
            ...
            messages: [{ role: 'user', content: [
                { type: 'text', text: 'Moderate this WhatsApp sticker. Reply with exactly YES only if it is sexually explicit, pornographic, or clearly depicts nudity/genitals; otherwise reply NO.' },
...
const enforceGroupFilters = async (sock, from, msg) => {
    ...
    if (badwordSettings.antinsfw && type === 'stickerMessage' && await classifySexualSticker(media, msg)) {
