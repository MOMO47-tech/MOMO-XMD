...
const enforceAntispam = async (sock, from, msg, body) => {
    ...
    try {
        await sock.sendMessage(from, { delete: msg.key });
        if (normalizedSender && typeof sock.updateBlockStatus === 'function') await sock.updateBlockStatus(normalizedSender, 'block');
        if (typeof sock.reportMessage === 'function') await sock.reportMessage(msg.key, true);
