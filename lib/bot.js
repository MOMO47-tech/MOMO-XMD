...
const COMMAND_STYLES = {
  ...
  online: { help: ['╭━━❐━⪼', '┇', '┇', '┇ ✦ online on', '┇ ✦ online off', '╰━━❑━⪼'], success: ['╭◆', '│ ✅ ONLINE ${state}', '│ ◆ setting saved', '╰◆'] },
...
const renderCommandHelp = command => {
    ...
    return renderCommandFrame(COMMAND_STYLES[command]?.help || ['╭◆', `│ ${command}`, '╰◆']);
};
const renderCommandSuccess = (command, state) => {
    const lines = (COMMAND_STYLES[command]?.success || ['╭◇', `│◇ ${command.toUpperCase()} ${state}`, '╰◇']).map(line => line.replace(/\$\{state\}/g, state));
    const target = lines.findIndex(line => line.includes(state));
    if (target >= 0) lines[target] = lines[target].replace(/^([│┇])\s*/, '$1 ✅ ');
    const footer = command === 'online' ? '> ❑ Powered by MOMO47 ❑' : '> Powered by MOMO47';
    return `${lines.join('\n')}\n${footer}`;
};
...
const renderAntilinkSuccess = (action, state) => `${renderAntilinkFrame(ANTILINK_STYLES[action].success, { state }, '> ✦ Powered by MOMO47 ✦')}\n✅`;
