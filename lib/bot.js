...
const renderCommandSuccess = (command, state) => {
    const style = COMMAND_STYLES[command];
    const lines = (style?.success || ['╭◇', `│◇ ${command.toUpperCase()} ${state}`, '╰◇']).map(line => line.replace(/\$\{state\}/g, state)).filter(line => !/setting\s+(saved|updated)|status\s+updated|reaction\s+setting\s+saved/i.test(line));
