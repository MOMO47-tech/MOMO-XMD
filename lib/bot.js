    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        switch (command) {
            ...
            case "tosgroup": {
                ...
                const quoted = findQuotedMessage(msg.message);
                const target = command === 'tostatus' ? 'status@broadcast' : from;
                const statusOptions = command === 'tostatus' ? {} : undefined;
                ...
                try {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        frame.border[0],
                        `${frame.border[1]}${frame.symbol} ${command === 'tostatus' ? 'STATUS' : 'GROUP MESSAGE'} SENT SUCCESSFUL ✅`,
                        frame.border[2]
