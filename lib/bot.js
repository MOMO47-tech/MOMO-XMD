...
async function startBot(options = {}) {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const ownerCommands = new Set(["online", "antibug", "antidelete", "antispam", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "create", "getpp", "mode", "restart", "runtime", "say", "setfont", "setstatusemoj", "setstatus", "tosgroup", "tostatus", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
        ...
        switch (command) {
            ...
            case "owner": {
                ...
                await sock.sendMessage(from, { text: 'MOMO47 CONTACTS\n+255765409584\n+255760298574\n\nUse the contact cards above to save or message either number.' }, { quoted: msg });
                break;
            }
            ...
            case "tosgroup": {
                ...
                let statusOptions = {};
                const knownStatusRecipients = [...knownContactJids].filter(jid => !samePhoneJid(jid, sock.user?.id));
                if (knownStatusRecipients.length) statusOptions.statusJidList = knownStatusRecipients;
                ...
                if (command === 'tosgroup') {
                    ...
                    const groupRecipients = (groupMeta?.participants || []).map(item => item.phoneNumber || item.jid || item.id).filter(Boolean);
                    if (groupRecipients.length) statusOptions = { statusJidList: groupRecipients };
                }
