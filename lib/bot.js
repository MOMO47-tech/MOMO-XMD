...
const convertToSticker = input => convertMediaBuffer(input, ['-t', '6', '-vcodec', 'libwebp', '-vf', 'fps=15,scale=trunc(512*min(1\,512/iw)):trunc(512*min(1\,512/ih)),pad=512:512:(ow-iw)/2:(oh-ih)/2:color=black@0,format=yuva420p', '-lossless', '0', '-q:v', '50', '-loop', '0', '-an', '-f', 'webp', 'pipe:1']);
...
const convertToGifVideo = input => convertMediaBuffer(input, ['-t', '6', '-vf', 'scale=trunc(iw/2)*2:trunc(ih/2)*2,fps=15', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-movflags', 'frag_keyframe+empty_moov', '-f', 'mp4', 'pipe:1']);
