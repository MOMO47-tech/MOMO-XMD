...
try {
    if (fs.existsSync(runtimeSettingsPath)) {
        ...
        for (const setting of ['antibug', 'antispam', 'antimention', 'antitag', 'antisticker', 'antigif']) {
...
const hasUnsafeLink = body => {
    const matches = String(body || '').match(/(?:https?:\/\/|www\.)[^\s]+/gi) || [];
...
const enforceAntispam = async (sock, from, msg, body) => {
    ...
    try {
        ...
        if (normalizedSender && typeof sock.updateBlockStatus === 'function') actions.push(sock.updateBlockStatus(normalizedSender, 'block'));
...
async function startBot(options = {}) {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const ownerCommands = new Set(["online", "antibug", "antispam", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "getpp", "mode", "restart", "runtime", "setfont", "setstatusemoj", "setstatus", "tostatus", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
