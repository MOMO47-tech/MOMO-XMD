...
const decoratedFooter = (lines, variant = 'generic', preferredSymbol = '') => {
    ...
    const symbol = preferredSymbol || palette.find(candidate => !body.includes(candidate)) || palette[0] || '◈';
...
const renderCommandFrame = (lines, values = {}, footer = '', variant = 'generic', footerSymbol = '') => {
    ...
    return resolved.map(line => `*${line}*`).join('\n') + `\n${decoratedFooter(resolved, variant, footerSymbol)}`;
};
...
const renderCommandHelp = command => {
    ...
    const sourceBottom = source.at(-1);
    const inner = source.slice(1, -1).map((line, index) => {
        ...
        const symbol = declaredSymbol || uniqueCommandFrame(`${command}:HELP:${state}`, 'HELP').symbol;
        const linePrefix = String(line).match(/^[┇│┃]/)?.[0] || frame.border[1];
        return `${linePrefix}${symbol} ${text}`;
    }).filter(Boolean);
    return `*Example*\n${renderCommandFrame([sourceTop, ...inner, sourceBottom], {}, style?.helpFooter || frame.footer, 'help', HELP_FOOTER_SYMBOLS[command] || '◇')}`;
};
...
const renderCommandSuccess = (command, state) => {
    ...
    const innerSymbol = frame.symbol === borderSymbol ? '◇' : frame.symbol;
    ...
    return `${top}\n${line}\n${bottom}\n${decoratedFooter([frame.border[0], frame.border[1], innerSymbol, frame.border[2]], 'success', SUCCESS_FOOTER_SYMBOLS[command]?.[normalizedState] || '★')}`;
};
...
const renderAntilinkFrame = (lines, values = {}, footer = '> ◇ Powered by MOMO47 ◇', variant = 'success') => {
    const resolved = lines.map(line => String(line).replace(/\$\{(\w+)\}/g, (_, key) => values[key] ?? ''));
    return renderCommandFrame(resolved, {}, footer, variant, variant === 'help' ? '◇' : '★');
};
...
const renderAntilinkSuccess = (action, state) => {
    ...
    const innerSymbol = style.symbol === borderSymbol || style.symbol === '◇' ? '★' : style.symbol;
    const lines = [
        top,
