    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        switch (command) {
            ...
            case "tosgroup": {
                ...
                let statusOptions = {
                    statusJidList: [...knownContactJids].filter(jid => !samePhoneJid(jid, sock.user?.id))
                };
                ...
                if (command === 'tosgroup') {
                    ...
                    statusOptions = { statusJidList: (groupMeta?.participants || []).map(item => item.jid || item.id).filter(Boolean) };
                }
            ...
            case "say": {
                ...
                try {
                    const audio = await convertToWhatsAppVoice(await synthesizeSpeech(caption));
                    await sock.sendMessage(from, { audio, mimetype: 'audio/ogg; codecs=opus', ptt: true }, { quoted: msg });
                } catch (error) {
            ...
            case "listcode": {
                ...
                try {
                    const metadata = await getCachedGroupMetadata(sock, from);
                    const members = (metadata?.participants || []).map(item => item.jid || item.id).filter(Boolean);
                    const sections = [];
