...
async function followNewsletterWithRetry(sock, channelValue) {
    ...
    const candidates = [jid, rawId];

    // Resolve the link first when the server supports newsletter metadata. Some
    // WhatsApp Web builds accept only the canonical ID returned by this query.
    if (typeof sock.newsletterMetadata === 'function') {
        try {
            const metadata = await sock.newsletterMetadata('jid', jid);
            if (metadata?.id) candidates.unshift(metadata.id);
        } catch (error) {
            console.warn(`[BOT] Channel metadata lookup failed for ${jid}:`, error?.message || error);
        }
    }
