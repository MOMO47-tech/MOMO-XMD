...
const decoratedFooter = (lines, variant = 'generic') => {
    ...
    const available = palette.filter(symbol => !body.includes(symbol));
    const symbol = available[0] || palette[0] || '◈';
    return `> ${symbol} Powered by MOMO47 ${symbol}`;
};
const renderCommandFrame = (lines, values = {}, footer = '', variant = 'generic') => {
...
const renderCommandHelp = command => {
    ...
    return `*Example*\n${renderCommandFrame([sourceTop, ...inner, sourceBottom], {}, style?.helpFooter || frame.footer, 'help')}`;
};
...
const renderCommandSuccess = (command, state) => {
    ...
    if (style) {
        ...
        return renderCommandFrame(lines, {}, style.footer, 'success');
    }
    ...
    return `${top}\n${line}\n${bottom}\n${decoratedFooter([frame.border[0], frame.border[1], innerSymbol, frame.border[2]], 'success')}`;
};
...
const renderAntilinkFrame = (lines, values = {}, footer = '> ◇ Powered by MOMO47 ◇', variant = 'success') => {
    const resolved = lines.map(line => String(line).replace(/\$\{(\w+)\}/g, (_, key) => values[key] ?? ''));
    return renderCommandFrame(resolved, {}, footer, variant);
};
