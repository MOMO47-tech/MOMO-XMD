...
const decoratedFooter = lines => {
    const body = Array.isArray(lines) ? lines.join('') : String(lines || '');
    const available = FOOTER_SYMBOLS.filter(symbol => !body.includes(symbol));
    const left = available[0] || '◈';
    const right = available[1] || (left === '◈' ? '✦' : '◈');
    return `> ${left} Powered by MOMO47 ${right}`;
};
const renderCommandFrame = (lines, values = {}, footer = '') => {
...
const renderCommandHelp = command => {
    ...
    return `*Example*\n${renderCommandFrame([sourceTop, ...inner, sourceBottom], {}, style?.helpFooter || frame.footer)}`;
};
...
const renderCommandSuccess = (command, state) => {
    ...
    if (style) {
        ...
        const lines = [border[0], `${border[1]}${style.symbol} ${style.title}`, border[2]];
    ...
    return `${top}\n${line}\n${bottom}\n${decoratedFooter([frame.border[0], frame.border[1], innerSymbol, frame.border[2]])}`;
};
...
const renderAntilinkSuccess = (action, state) => {
    ...
    const innerSymbol = style.symbol === borderSymbol ? '◇' : style.symbol;
    const lines = [
        top,
