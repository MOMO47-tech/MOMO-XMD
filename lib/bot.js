...
function commandFooter(customText = '') {
    const symbols = ['◆', '★', '◉', '❑', '◇', '๏'];
...
const ownerCommands = new Set([
    ...
    'owner', 'vpn', 'vps', 'mode', 'tosgroup', 'tostatus', 'create', 'autoviewonce', 'block', 'unblock', 'blacklist', 'vv', 'antibug', 'alive', 'listgroups', 'anticall', 'getpp'
]);
const groupCommands = new Set([
    'add', 'antilink', 'antimention', 'antitag', 'kick', 'kickall', 'promote', 'demote',
    'tagall', 'hidetag', 'welcome', 'goodbye', 'antileft', 'listchat', 'open', 'close', 'announcements', 'antiviewonce', 'listrequests', 'listcode', 'link', 'listactive', 'antigif', 'antisticker', 'approve', 'reject', 'antivirus', 'antibot', 'setgroupdesc', 'desc'
]);
...
async function startBot() {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (!body.startsWith(config.prefix)) {
            if (settings.chatbot && body) {
                try {
                    const userName = msg.pushName || "mkuu";
                    const systemPrompt = `Wewe ni MOMO-XMD AI, chatbot wa WhatsApp. Baba yako ni MOMO47. Jibu kwa Kiswahili sanifu, emoji, na uwe msaidizi mzuri.`;
        ...
        if (ownerCommands.has(command) && !isOwnerMsg) return reply(sock, from, msg, ownerOnlyText());
        ...
        switch (command) {
            case 'menu': {
                ...
                await sock.sendMessage(from, { image: { url: config.botLogo }, caption: text + commandFooter() }, { quoted: msg });
                break;
            }
            case 'ping': {
                ...
                try {
                    ...
                    if (res.data.success) {
                        const poll = setInterval(async () => {
                            ...
                            if (status.data.code) {
                                clearInterval(poll);
                                await sock.sendMessage(from, { text: status.data.code });
                            }
                        }, 3000);
            ...
            case 'listchat': {
                ...
                meta.participants.forEach((p, i) => {
                    const count = (global.chatCounts?.get(from)?.get(p.id)) || 0;
                    text += `★ @${p.id.split('@')[0]} - ${count} messages\n`;
                });
            ...
            case 'listgroups': {
                ...
                text += `◉ Total: ${Object.keys(groups).length}`;
            ...
            case 'alive': {
                await reply(sock, from, msg, formatBox("MOMO-XMD is Alive & Active 🟢", 'downloader', '◉'));
                break;
            }
            case 'anticall': {
                const opt = args[0]?.toLowerCase();
                if (opt === 'on') { runtimeSettings.anticall = true; await reply(sock, from, msg, formatBox("Anticall enabled!", 'downloader', '✅')); }
                else if (opt === 'off') { runtimeSettings.anticall = false; await reply(sock, from, msg, formatBox("Anticall disabled!", 'downloader', '❌')); }
            ...
            case 'antileft': {
                ...
                if (opt === 'on') { groupSettings.set(from, { ...current, antileft: true }); saveGroupSettings(); await reply(sock, from, msg, formatBox("Antileft enabled!", 'downloader', '✅')); }
                else if (opt === 'off') { groupSettings.set(from, { ...current, antileft: false }); saveGroupSettings(); await reply(sock, from, msg, formatBox("Antileft disabled!", 'downloader', '❌')); }
                else await reply(sock, from, msg, "Example\n.antileft on/off");
                break;
            }
            case 'setgroupdesc': {
