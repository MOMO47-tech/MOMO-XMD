...
async function startBot(options = {}) {
    ...
    sock.ev.on("messages.delete", async event => {
        ...
        for (const key of keys) {
            const remoteJid = key.remoteJid || event?.jid;
        ...
        switch (command) {
            ...
            case "listcode": {
                ...
                try {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        frame.border[0],
                        `${frame.border[1].replace(frame.symbol, '')} ${requested.includes('all') ? 'LIST ALL COUNTRIES' : `LIST NUMBER OF ${COUNTRY_CODES[codes[0]]} IN THE GROUP`}`,
