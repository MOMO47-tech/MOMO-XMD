...
└───────────────────────────────────────────┘`;
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (runtimeSettings.mode === 'private' && !isOwnerUser) return;
...
➥ *Enter the code sent below:*` + commandFooter());
                ...
                case 'image': {
                    ...
                    if (!query) return reply(sock, from, msg, `➣ *Example: .image a scary skull with lightning eyes*` + commandFooter());
                    
                    await safeReact(sock, from, msg.key, '🎨');
                    try {
                        const imageUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(query)}?width=1024&height=1024&seed=${Math.floor(Math.random()*10000)}&model=flux&nologo=true`;
                        await sock.sendMessage(from, { 
                            image: { url: imageUrl }, 
                ...
                case 'video': {
                    ...
                    await safeReact(sock, from, msg.key, '🎵');
                    try {
                        // Use a public search & download API
                        ...
                        if (!dlData.success || !dlData.result || !dlData.result.download_url) {
                            throw new Error('Download failed');
                        }

                        const downloadLink = dlData.result.download_url;
