...
const COMMAND_STYLES = {
  ...
  autoreact: { help: ['╭◆', '│◇ 𝙰𝚄𝚃𝙾𝚁𝙴𝙰𝙲𝚃', '│', '│ ★ autoreact on', '│ ★ autoreact off', '╰◆'], success: ['╭━━❖━⪼', '┇❖ AUTOREACT ${state}', '┇❖ reaction setting saved', '╰━━❖━⪼'], helpFooter: '> ◈ Powered by MOMO47 ◈', successFooter: '> ❑ Powered by MOMO47 ❑' },
  chatbot: { help: ['╭◆', '│◉ 𝙲𝙷𝙰𝚃𝙱𝙾𝚃', '│', '│ ๏ chatbot on', '│ ๏ chatbot off', '╰◆'], success: ['╭━━◉━⪼', '┇◉ CHATBOT ${state}', '┇◉ setting saved', '╰━━◉━⪼'], helpFooter: '> ◇ Powered by MOMO47 ◇', successFooter: '> ★ Powered by MOMO47 ★' },
  online: { help: ['╭━━❐━⪼', '┇', '┇', '┇ ✦ online on', '┇ ✦ online off', '╰━━❑━⪼'], success: ['╭◆', '│ ONLINE ${state}', '│ ◇ setting saved', '╰◆'], helpFooter: '> ❖ Powered by MOMO47 ❖', successFooter: '> ★ Powered by MOMO47 ★' },
  autolikestatus: { help: ['╭◆', '│❖ 𝙰𝚄𝚃𝙾𝙻𝙸𝙺𝙴𝚂𝚃𝙰𝚃𝚄𝚂', '│', '│ ❖ autolikestatus on', '│ ❖ autolikestatus off', '╰◆'], success: ['╭━━๏━⪼', '┇๏ AUTOLIKESTATUS ${state}', '┇๏ setting saved', '╰━━๏━⪼'], helpFooter: '> ◈ Powered by MOMO47 ◈', successFooter: '> ❑ Powered by MOMO47 ❑' },
  autosavestatus: { help: ['╭◆', '│❑ 𝙰𝚄𝚃𝙾𝚂𝙰𝚅𝙴𝚂𝚃𝙰𝚃𝚄𝚂', '│', '│ ❑ autosavestatus on', '│ ❑ autosavestatus off', '╰◆'], success: ['╭━━✦━⪼', '┇✦ AUTOSAVESTATUS ${state}', '┇✦ setting saved', '╰━━✦━⪼'], helpFooter: '> ◇ Powered by MOMO47 ◇', successFooter: '> ★ Powered by MOMO47 ★' },
  autoviewonce: { help: ['╭◆', '│◇ 𝙰𝚄𝚃𝙾𝚅𝙸𝙴𝚆𝙾𝙽𝙲𝙴', '│', '│ ◇ autoviewonce on', '│ ◇ autoviewonce off', '╰◆'], success: ['╭━━❖━⪼', '┇❖ AUTOVIEWONCE ${state}', '┇❖ setting saved', '╰━━❖━⪼'], helpFooter: '> ✦ Powered by MOMO47 ✦', successFooter: '> ◈ Powered by MOMO47 ◈' },
  autorecording: { help: ['╭━━❐━⪼', '┇๏ 𝙰𝚄𝚃𝙾𝚁𝙴𝙲𝙾𝚁𝙳𝙸𝙽𝙶', '┇', '┇ ๏ autorecording on', '┇ ๏ autorecording off', '╰━━❑━⪼'], success: ['╭◆', '│✦ AUTORECORDING ${state}', '│✦ setting saved', '╰◆'], helpFooter: '> ❖ Powered by MOMO47 ❖', successFooter: '> ◇ Powered by MOMO47 ◇' },
  autotyping: { help: ['╭◆', '│★ 𝙰𝚄𝚃𝙾𝚃𝚈𝙿𝙸𝙽𝙶', '│', '│ ★ autotyping on', '│ ★ autotyping off', '╰◆'], success: ['╭━━◈━⪼', '┇◈ AUTOTYPING ${state}', '┇◈ setting saved', '╰━━◈━⪼'], helpFooter: '> ❑ Powered by MOMO47 ❑', successFooter: '> ✦ Powered by MOMO47 ✦' }
};
...
const renderCommandHelp = command => {
    if (command === 'online') return `${COMMAND_STYLES.online.help.join('\n')}\n> ✦ Powered by MOMO47 ✦`;
...
const renderCommandSuccess = (command, state) => {
    const style = COMMAND_STYLES[command];
    const lines = (style?.success || ['╭◇', `│◇ ${command.toUpperCase()} ${state}`, '╰◇']).map(line => line.replace(/\$\{state\}/g, state));
    const target = lines.findIndex(line => line.includes(state));
    if (target >= 0) {
        const prefix = lines[target].match(/^([│┇]\s*[^A-Za-z0-9]*)/)?.[1] || '│ ';
        lines[target] = `${prefix}${command.toUpperCase()} ${state} SETTING SUCCESSFUL ✅`;
    }
    const footer = style?.successFooter || '> Powered by MOMO47';
    return `${lines.join('\n')}\n${footer}`;
};
...
const renderAntilinkHelp = () => ['Example', '╭━━✧━⪼', '┇◇ antilink delete on/off', '┇◇ antilink warn on/off', '┇◇ antilink kick on/off', '╰━━✧━⪼', '> ◇ Powered by MOMO47 ◇'].join('\n');
const renderAntilinkSuccess = (action, state) => {
    const lines = ANTILINK_STYLES[action].success.map(line => line.replace(/\$\{state\}/g, state));
    const target = lines.findIndex(line => line.includes(state));
    if (target >= 0) {
        const prefix = lines[target].match(/^([│┇]\s*[^A-Za-z0-9]*)/)?.[1] || '┇ ';
