...
for (const style of Object.values(COMMAND_STYLES)) {
    ...
    style.helpFooter = `> ${symbol} Powered by MOMO47 ${symbol}`;
}
const renderCommandFrame = (lines, values = {}, footer = '> Powered by MOMO47') => {
    ...
    return resolved.map(line => `*${line}*`).join('\n') + `\n*${String(footer || '> ❖ Powered by MOMO47 ❖')}*`;
};
const makeSuccessStyle = (border, symbol, footer, command, state) => ({ border, symbol, footer: `> ${symbol} Powered by MOMO47 ${symbol}`, title: `${command.toLowerCase()} ${state.toLowerCase()} setting successful ✅` });
...
const renderAntilinkSuccess = (action, state) => {
    ...
    const lines = [
        top,
        `${linePrefix}${style.symbol} ✅ antilink ${action.toLowerCase()} ${String(state).toLowerCase()} setting successful`,
        bottom
    ];
