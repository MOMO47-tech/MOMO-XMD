...
async function startBot() {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        try {
            ...
            if (!msg) return;
            console.log(`[UPSERT] type=${m.type} count=${m.messages?.length || 0}`);
            ...
            console.log(`[MESSAGE_DEBUG] From: ${from} | Sender: ${senderJid} | Body: "${body}" | isOwner: ${isOwnerMsg}`);
...
if (!body.startsWith(config.prefix)) {
    if (body && !msg.key.fromMe) {
        console.log(`[IGNORE_DEBUG] Message doesn't start with prefix "${config.prefix}": "${body.substring(0, 20)}..."`);
    }
    if (settings.chatbot && body && !msg.key.fromMe) {
        try {
            const systemPrompt = `Wewe ni MOMO-XMD AI, chatbot wa WhatsApp. Baba yako ni MOMO47. Jibu kwa Kiswahili sanifu, emoji, na uwe msaidizi mzuri. Jibu kwa Kiingereza pekee.`;
    ...
    return;
        ...
        console.log(`[COMMAND CHECK] Parsed command: "${command}" with args:`, args);
