...
const convertToSticker = input => convertMediaBuffer(input, ['-vcodec', 'libwebp', '-vf', 'scale=512:512:force_original_aspect_ratio=decrease,fps=15,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=white@0', '-loop', '0', '-an', '-f', 'webp', 'pipe:1']);
const convertToVideo = input => convertMediaBuffer(input, ['-an', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-movflags', 'frag_keyframe+empty_moov', '-f', 'mp4', 'pipe:1']);
const convertToGifVideo = input => convertMediaBuffer(input, ['-an', '-vf', 'fps=12,scale=480:-2:force_original_aspect_ratio=decrease', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-movflags', 'frag_keyframe+empty_moov', '-f', 'mp4', 'pipe:1']);
...
const downloadQuotedMedia = async (sock, msg, quoted, quotedContent, quotedType) => {
    const media = quotedContent?.[quotedType];
    if (!media) throw new Error('quoted media payload is missing');
    const key = {
    ...
    const source = { key, message: quotedContent, messageTimestamp: msg.messageTimestamp };
    try {
        const kind = quotedType === 'stickerMessage' ? 'sticker' : quotedType === 'videoMessage' || quotedType === 'gifMessage' ? 'video' : 'image';
        const stream = await downloadContentFromMessage(media, kind, {});
        const directChunks = [];
        for await (const chunk of stream) directChunks.push(chunk);
        const directBuffer = Buffer.concat(directChunks);
        if (await isDecodableMediaBuffer(directBuffer, quotedType)) return directBuffer;
        console.warn('[MEDIA] direct quoted stream was not a valid media buffer');
    } catch (error) {
        console.warn('[MEDIA] direct quoted download failed:', error?.message || error);
    }
    try {
        ...
        if (await isDecodableMediaBuffer(buffer, quotedType)) return buffer;
    } catch (error) {
        console.warn('[MEDIA] quoted download fallback:', error?.message || error);
    }
    const kind = quotedType === 'stickerMessage' ? 'sticker' : quotedType === 'imageMessage' ? 'image' : 'video';
    const stream = await downloadContentFromMessage(media, kind);
    const chunks = [];
    for await (const chunk of stream) chunks.push(chunk);
    const buffer = Buffer.concat(chunks);
    if (!(await isDecodableMediaBuffer(buffer, quotedType))) throw new Error('quoted media download returned invalid data; reply to the original sticker/video');
    return buffer;
};
...
async function startBot(options = {}) {
    ...
    sock.ev.on('group-participants.update', async (update) => {
        ...
        if ((settings.antileft || settings.alive) && ['remove', 'leave'].includes(action) && typeof sock.groupParticipantsUpdate === 'function') {
            for (const participant of participants) {
                const user = participant?.id || participant?.jid || participant;
                const shouldRestore = settings.antileft || (settings.alive && samePhoneJid(user, settings.aliveOwner || config.botOwner));
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        switch (command) {
            ...
            case "togif": {
                ...
                const quotedType = Object.keys(quotedContent)[0];
                const media = quotedContent[quotedType] || {};
                const sourceAllowed = command === 'tosticker'
                    ? ['imageMessage', 'videoMessage', 'gifMessage'].includes(quotedType)
                    : ['stickerMessage', 'videoMessage', 'gifMessage'].includes(quotedType);
                if (!quoted?.message || !sourceAllowed) {
                    const conversionFrame = uniqueCommandFrame(`${command}:HELP`, 'HELP');
                    const sourceText = command === 'tosticker' ? 'reply to a video or gif' : command === 'tovideo' ? 'reply to a sticker or gif' : 'reply to a sticker or video';
                    await sock.sendMessage(from, { text: `Example\n${renderPlainCommandFrame([
                ...
                try {
                    if (!quoted.stanzaId) throw new Error('reply to the original media message, not a forwarded copy');
                    const buffer = await downloadQuotedMedia(sock, msg, quoted, quotedContent, quotedType);
