...
const chatbotAnswer = async (chatId, prompt) => {
    ...
    try {
        ...
        for (let attempt = 0; attempt < 2; attempt += 1) {
            ...
            temperature: 0.7,
            ...(model.startsWith('gpt-5') ? { max_completion_tokens: 700 } : { max_tokens: 700 })
                }, { headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, timeout: 60000 });
                break;
            } catch (error) {
                lastError = error;
                if (attempt === 0 && (!error?.response || error.response.status >= 500 || error.response.status === 429)) await delay(1200);
                else break;
            }
        }
...
const downloadQuotedMedia = async (msg) => {
    ...
    if (!looksLikeMediaBuffer(buffer, mediaType)) {
        const source = {
    ...
    if (!looksLikeMediaBuffer(buffer, mediaType)) throw new Error('quoted media download returned invalid or empty data');
