...
async function startBot(options = {}) {
    ...
    const recoverDeletedKey = async key => {
        ...
        try {
            ...
            if (STATUS_MEDIA_TYPES.has(contentType)) {
                ...
                const deletedText = content.conversation || content.extendedTextMessage?.text || media.caption;
                if (deletedText) await sock.sendMessage(ownerJid, { text: String(deletedText) });
            }
        } catch (error) {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (from === 'status@broadcast' && runtimeSettings.antimention && isMentionedMessage(msg) && !msg.key.fromMe) {
            try { await sock.sendMessage('status@broadcast', { delete: msg.key }); } catch (error) {
                console.warn('[ANTIMENTION] status delete failed:', error?.message || error);
            }
            continue;
        }
        ...
        const ownerCommands = new Set(["online", "antibug", "antidelete", "antispam", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "create", "getpp", "mode", "restart", "setfont", "setstatusemoj", "setstatus", "tosticker", "tovideo", "togif", "tosgroup", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
                ...
                try {
                    ...
                    if (groupQuoted?.message) {
                        ...
                        if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(quotedType)) {
                            ...
                            await sock.sendMessage(target, { text: groupCaption || quotedContent.conversation || quotedContent.extendedTextMessage?.text || '' }, statusOptions);
                        }
                    } else {
                        await sock.sendMessage(target, { text: groupCaption }, statusOptions);
                    }
