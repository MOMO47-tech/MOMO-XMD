...
const looksLikeMediaBuffer = (buffer, type) => {
    ...
    return buffer.subarray(4, 8).toString() === 'ftyp' || buffer.subarray(0, 4).toString() === 'RIFF' || buffer.subarray(0, 4).toString() === 'OggS';
};
const downloadQuotedMedia = async (sock, msg, quoted, quotedContent, quotedType) => {
    ...
    try {
        ...
        if (looksLikeMediaBuffer(directBuffer, quotedType)) return directBuffer;
    ...
    try {
        ...
        if (looksLikeMediaBuffer(buffer, quotedType)) return buffer;
    } catch (error) {
    ...
    if (!looksLikeMediaBuffer(buffer, quotedType)) throw new Error('quoted media download returned invalid data; reply to the original sticker/video');
    return buffer;
};
...
async function startBot(options = {}) {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const ownerCommands = new Set(["online", "antibug", "antidelete", "antispam", "alive", "left", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "create", "getpp", "mode", "restart", "setfont", "setstatusemoj", "setstatus", "tosticker", "tovideo", "togif", "tosgroup", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
        ...
        switch (command) {
            ...
            case "tosgroup": {
                ...
                try {
                    ...
                    if (typeof sock.sendGroupStatus !== 'function' && typeof sock.stianStatus?.sendGroupStatus !== 'function') {
                        throw new Error('This Baileys build has no group-status API; tosgroup cannot be sent as a real group status.');
                    }
                    const sendGroupStatus = sock.sendGroupStatus
                        ? (payload => sock.sendGroupStatus([from], payload))
                        : (payload => sock.stianStatus.sendGroupStatus(from, payload));
            ...
            case "togif": {
                ...
                const sourceAllowed = command === 'tosticker'
                    ? ['videoMessage', 'gifMessage'].includes(quotedType)
