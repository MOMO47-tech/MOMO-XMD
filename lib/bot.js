...
const ANTILINK_STYLES = {
  delete: { help: ['╭━━✧━⪼', '┇◇ antilink delete on', '┇◇ antilink delete off', '╰━━✧━⪼'], success: ['╭━━✦━⪼', '┇✦ ANTILINK DELETE ${state}', '┇✦ delete setting updated', '╰━━✧━⪼'] },
  warn: { help: ['╭━━◈━⪼', '┇◈ antilink warn on', '┇◈ antilink warn off', '╰━━◈━⪼'], success: ['╭━━⬡━⪼', '┇⬡ ANTILINK WARN ${state}', '┇⬡ warning setting updated', '╰━━⬢━⪼'] },
  kick: { help: ['╭━━❖━⪼', '┇❖ antilink kick on', '┇❖ antilink kick off', '╰━━❖━⪼'], success: ['╭━━✺━⪼', '┇✺ ANTILINK KICK ${state}', '┇✺ kick setting updated', '╰━━✹━⪼'] }
};
...
const enforceAntilink = async (sock, from, msg, body) => {
    ...
    try {
        ...
        if (action === 'kick') {
            ...
            await sock.sendMessage(from, renderAntilinkActionNotice('kick', senderJid), { quoted: msg });
            return true;
        }
            ...
            case "menu":
                ...
                try {
                    ...
                    await Promise.race([
                        ...
                        new Promise((_, reject) => setTimeout(() => reject(new Error('menu image send timeout')), 15000))
                    ]);
