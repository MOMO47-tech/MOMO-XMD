...
const protectFontText = (text, transform) => {
    ...
    const masked = source.replace(/https?:\/\/\S+|[A-Z0-9]{4,}(?:-[A-Z0-9]+)+/g, match => {
        const token = `__MOMO_FONT_${protectedParts.length}__`;
        protectedParts.push([token, match]);
        return token;
    });
    let output = transform(masked);
    for (const [token, original] of protectedParts) output = output.replace(token, original);
    return output;
};
...
const runtimeSettings = { mode: config.mode || "public", anticall: false, chatbot: false, autoviewstatus: false, autolikestatus: false, autosavestatus: false, autoviewonce: false, autoreact: false, autorecording: false, autotyping: false, online: false, antibug: false, antiforeign: false, statusEmoji: '❤️', font: 0 };
...
const saveRuntimeSettings = () => {
    try {
        ...
        fs.writeFileSync(runtimeSettingsPath, JSON.stringify({ statusEmoji: runtimeSettings.statusEmoji, font: runtimeSettings.font }, null, 2));
    } catch (error) {
...
async function startBot(options = {}) {
    ...
    sock.ev.on('group-participants.update', async (update) => {
        ...
        if (!id || !Array.isArray(participants) || !['add', 'remove', 'leave'].includes(action)) return;
        try {
            let metadata;
            try {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const ownerCommands = new Set(["online", "antibug", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "getpp", "mode", "restart", "runtime", "setfont", "setstatusemoj", "setstatus", "tostatus", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
        ...
        switch (command) {
            ...
            case "setfont": {
                ...
                const fontHelp = () => {
                    ...
                    const lines = [
                        frame.border[0],
                        `${frame.border[1]}${frame.symbol} setfont 1 - default`,
                        ...FONT_NAMES.slice(1).map((name, index) => `${frame.border[1]}${frame.symbol} setfont ${index + 1} - ${name}`),
                ...
                if (requested === 'off' || requested === '0') {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        frame.border[0], `${frame.border[1]}${frame.symbol} setfont ${selected} ${FONT_NAMES[selected]} successful ✅`, frame.border[2]
                    ], {}, '', 'success', frame.symbol) }, { quoted: msg });
                }
                break;
            }
                ...
                if (command === 'tagall') {
                    ...
                    if (!caption) {
                        ...
                        await sock.sendMessage(from, { text: `Example\n${renderCommandFrame([
                            ...
                            `${helpFrame.border[1]}${helpFrame.symbol} tagall reply to a message and type tagall`,
                            helpFrame.border[2]
                        ], {}, '', 'help', helpFrame.symbol)}` }, { quoted: msg });
                        break;
                    }
                    ...
                    const tagLines = [
                        tagFrame.border[0],
                        `${tagFrame.border[1]}${tagFrame.symbol} tagall 🚨 ${caption}`,
                        `${tagFrame.border[1]}${tagFrame.symbol} `,
                        ...members.map(member => `${tagFrame.border[1]}${tagFrame.symbol} @${String(member).split('@')[0]}`),
                        tagFrame.border[2]
                    ];
                    await sock.sendMessage(from, {
                        text: renderCommandFrame(tagLines, {}, '', 'generic', tagFrame.symbol),
                        mentions: members
                    }, { quoted: msg });
                } else {
                    const caption = captionFromArgs || String(quotedText || '').trim();
            ...
            case "repo": {
                if (command === 'repo') {
                    ...
                    const repoLines = [
                        repoFrame.border[0],
                        `${repoFrame.border[1]}${repoFrame.symbol} MOMO-XMD REPOSITORY`,
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame(repoLines, {}, '', 'generic', repoFrame.symbol) }, { quoted: msg });
                    break;
                }
                ...
                const listLines = [
                    ...
                    ...channelLinks.flatMap((link, index) => [
                        ...
                        `${listFrame.border[1]}${listFrame.symbol} ${link}`,
                        `${listFrame.border[1]}${listFrame.symbol} `
                    ]),
                    `${listFrame.border[1]}${listFrame.symbol} GROUP`,
                    `${listFrame.border[1]}${listFrame.symbol} ${groupLink}`,
                    `${listFrame.border[1]}${listFrame.symbol} command: .group`,
                    listFrame.border[2]
                ];
                await sock.sendMessage(from, { text: renderCommandFrame(listLines, {}, '', 'generic', listFrame.symbol) }, { quoted: msg });
                break;
            }
            ...
            case "pair": {
                ...
                await sock.sendMessage(from, { text: `${instructions[0]}\n${renderCommandFrame(instructions.slice(1), {}, '', 'generic', pairFrame.symbol)}` }, { quoted: msg });
