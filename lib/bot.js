...
const fitCommandBorders = rawLines => {
    ...
    const innerWidth = Math.max(...rawLines.slice(1, -1).map(visibleWidth), 1) + 2;
...
const renderCommandFrame = (lines, values = {}, footer = '> Powered by MOMO47') => {
    ...
    return fitCommandBorders(resolved).map(line => `*${line}*`).join('\n') + `\n*${footer}*`;
};
...
const renderCommandHelp = command => {
    ...
    const inner = source.slice(1, -1).map(line => {
        const cleaned = String(line).replace(/^[┇│┃]\s*/, '').trim();
        if (!cleaned) return null;
        const symbol = COMMAND_SYMBOLS.find(candidate => cleaned.startsWith(candidate)) || frame.symbol;
        const text = symbol === frame.symbol ? cleaned : cleaned.slice(Array.from(symbol).length).trim();
        return `${frame.border[1]}${symbol} ${text}`;
    }).filter(Boolean);
    return renderCommandFrame([frame.border[0], ...inner, frame.border[2]], {}, frame.footer);
};
...
const uniqueCommandFrame = (command, state) => {
    ...
    const isOnline = String(command).toLowerCase().startsWith('online');
    const symbols = isOnline ? COMMAND_SYMBOLS.filter(symbol => !['❑', '❐', '▣'].includes(symbol)) : COMMAND_SYMBOLS;
...
const permissionReply = (kind, message) => {
    const frame = uniqueCommandFrame(`permission:${kind}`, 'DENIED');
    return renderCommandFrame([
        frame.border[0],
        `${frame.border[1]}${frame.symbol} ${message}`,
        frame.border[2]
    ], {}, frame.footer);
};
const groupOnlyText = () => permissionReply('GROUP', 'Mkuu, amri hii inafanya kazi kwenye magroup pekee!');
        ...
        switch (command) {
            ...
            case "mode": {
                ...
                if (["public", "self"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        modeFrame.border[0],
                        `${modeFrame.border[1]}${modeFrame.symbol} Example`,
                ...
                if (command === "antilink") {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        ...
                        await sock.sendMessage(from, { text: renderCommandFrame([
                            ...
                            `${antiviewHelpFrame.border[1]}${antiviewHelpFrame.symbol} Example`,
