...
const ownerCommands = new Set([
    'autoviewstatus', 'autolikestatus', 'setstatus', 'autosavestatus', 'restart', 'update',
    'setfont', 'chatbot', 'autorecording', 'autotyping', 'alwaysonline', 'autoreact',
    'owner', 'pairing', 'channel', 'vpn', 'vps', 'mode', 'tosgroup', 'create'
]);
const groupCommands = new Set([
    'add', 'antilink', 'antigroupmention', 'kick', 'promote', 'demote',
    'tagall', 'hidetag', 'welcome', 'goodbye', 'open', 'close', 'announcements'
]);
...
async function reply(sock, jid, key, text) { return sock.sendMessage(jid, { text }, { quoted: key }); }
...
async function restoreCompactSession(sessionId, sessionPath) {
        ...
        try {
            ...
            for (const [relativeName, encoded] of Object.entries(payload.files)) {
                ...
                if (normalized.startsWith('../') || normalized.includes('/../') || path.posix.isAbsolute(normalized)) {
                    throw new Error('registry contained an unsafe auth path');
                }
                const destination = path.join(sessionPath, normalized);
                fs.mkdirSync(path.dirname(destination), { recursive: true });
                fs.writeFileSync(destination, Buffer.from(String(encoded), 'base64'));
            }
            if (!fs.existsSync(path.join(sessionPath, 'creds.json'))) throw new Error('registry missing creds.json');
            if (payload.fullNumber) {
                fs.writeFileSync(path.join(sessionPath, 'deployer.txt'), payload.fullNumber);
            }
            console.log('[SESSION] Compact Session ID restored successfully');
            return true;
        } catch (error) {
            lastError = error;
        } finally {
    ...
    console.log('[SESSION] Compact Session ID registry lookup failed:', lastError?.message || 'unknown error');
    return false;
}
...
async function startBot() {
    ...
    if (sessionId) {
        ...
        if (!sessionRestored && /^MOMO-XMD~[HV][A-Z0-9]{22}$/.test(sessionId)) {
            throw new Error('Compact Session ID registry was not reachable or has expired');
        }
    }
...
└───────────────────────────────────────────┘`;
        ...
        try {
            ...
            for (const num of participants) {
                ...
                if ((action === 'add' || action === 'join') && settings.welcome) {
                    const welcomeText = `➣ ${tag} Welcome to *${groupName}*, feel at home\n\n*Powered by MOMO-XMD* 🚀\n*Owner MOMO47* ☠️`;
                    console.log(`✨ Sending Welcome to ${num} in group ${id}`);
                    await sock.sendMessage(id, { text: welcomeText, mentions });
                } else if ((action === 'remove' || action === 'leave') && settings.goodbye) {
                    const goodbyeText = `➣ ${tag} Goodbye, welcome again to *${groupName}*, feel at home\n\n*Powered by MOMO-XMD* 🚀\n*Owner MOMO47* ☠️`;
                    console.log(`✨ Sending Goodbye to ${num} in group ${id}`);
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (msg.key.fromMe && !isSelfChat && !isOwnerMsg && !bodyCheck.startsWith(config.prefix)) return;
        ...
        if (runtimeSettings.autorecording || settings.autorecording) {
            ...
            if (runtimeSettings.chatbot && body && !body.startsWith(config.prefix)) {
            ...
            let aiReply = "";
            const systemPrompt = `You are MOMO-XMD, the ultimate high-intelligence AI chatbot, created by your father MOMO47. You are a genius in coding (all languages), science, history, life coaching, and relationship advice. Your intelligence level is equivalent to Manus AI. Always provide deep, structured, and insightful answers. If asked for code, provide full, functional examples in code blocks. Always identify as the child of MOMO47. You speak Swahili and English fluently.`;

            try {
                const res = await fetch(`https://text.pollinations.ai/${encodeURIComponent(body)}?system=${encodeURIComponent(systemPrompt)}&model=openai`);
                if (res.ok) {
                    const text = await res.text();
                    if (text && text.length > 5) aiReply = text.trim();
                }
            } catch (e) {
                console.log('[Pollinations AI Error]:', e.message);
            }

            if (!aiReply || aiReply.length < 5) {
                try {
                    const res2 = await fetch(`https://itzpire.com/ai/gpt?q=${encodeURIComponent(systemPrompt + "\nUser: " + body)}`);
                    if (res2.ok) {
                        const data = await res2.json();
                        aiReply = (data?.result || data?.data || "").trim();
                    }
                } catch (e) {
            ...
            if (!aiReply || aiReply.length < 5) {
                aiReply = `Mimi ni mtoto halisi wa MOMO47 ☠️. Kuhusu swali lako "${body}": Nimechanganua kwa umakini mkubwa. Nina uwezo kamili wa kukusaidia kwenye coding, maisha, mahusiano, na masomo. Nijulishe ni kipengele gani hasa ungependa nikuandalie au nikutatulie!`;
            }

            const prefix = isOwnerSender ? "➣ *MOMO-XMD AI (Baba):* " : "➣ *MOMO-XMD AI:* ";
            await reply(sock, from, msg, prefix + aiReply + commandFooter());
        }
        ...
        if (groupCommands.has(command)) {
            try { group = await getGroupContext(sock, msg); } catch (error) { return reply(sock, from, msg, groupOnlyText()); }
            if (['add', 'antilink', 'antigroupmention', 'kick', 'promote', 'demote', 'welcome', 'goodbye', 'open', 'close', 'announcements'].includes(command) && !group.isAdmin) {
        ...
        try {
            switch (command) {
                ...
                case 'ping': {
                    ...
                    await reply(sock, from, msg, `➣ *Speed: ${latency} ms* ⚡` + commandFooter());
                    break;
                }
                case 'runtime': {
                    await safeReact(sock, from, msg.key, '⏰');
                    ...
                    const runtime = `${days} days, ${hours} hours, ${minutes} minutes, ${secs} seconds`;
                    await reply(sock, from, msg, `➣ *Runtime: ${runtime}* ⏰` + commandFooter());
                    break;
                }
                case 'owner': {
...
➥ *Enter the code sent below:*` + commandFooter());
                ...
                case 'setfont': {
                    const value = String(args[0] || '').toLowerCase();
                    const fonts = ['bold', 'italic', 'mono', 'cursive', 'fantasy', 'script', 'gothic', 'bubbly', 'square', 'tiny', 'double', 'outline', 'medieval', 'shadow', 'inverted', 'off'];
                    if (!value || !fonts.includes(value)) {
                        const list = fonts.map(f => `➥ *${f}*`).join('\n');
                        return reply(sock, from, msg, `➣ *Example setfont bold/mono/cursive*\n\n${list}` + commandFooter());
                    }
                ...
                case 'add': {
                    ...
                    if (!/^\d{8,15}$/.test(number)) return reply(sock, from, msg, `➣ *Example: .add +255765409584*` + commandFooter());
                    await sock.groupParticipantsUpdate(from, [`${number}@s.whatsapp.net`], 'add');
                    await reply(sock, from, msg, `➣ _${number} were added successful_ ✅` + commandFooter());
                    break;
                }
                case 'kick':
                case 'promote':
                case 'demote': {
                    ...
                    const text = command === 'kick' ? `➣ _@${target.split('@')[0]} removed successful_ ✅` : command === 'promote' ? `➣ _Congratulations_ 🥳 _@${target.split('@')[0]} you have been promoted to admin by @${actor.split('@')[0]}_` : `➣ _Sad_ 😔 _@${target.split('@')[0]} you have been demoted by @${actor.split('@')[0]}_`;
                    await sock.sendMessage(from, { text, mentions: [target, actor] }, { quoted: msg });
                    break;
                }
                case 'tagall': {
                    await safeReact(sock, from, msg.key, '📢');
                ...
                case 'restart':
                    await reply(sock, from, msg, '*Bot restarting.......*' + commandFooter());
                ...
                case 'image': {
                    ...
                    await safeReact(sock, from, msg.key, '🎨');
                    try {
                        // Enhance prompt for better celebrity recognition
                        const enhancedPrompt = `High quality, realistic portrait of ${query}, professional photography, 8k resolution, detailed features`;
                ...
                case 'video': {
                    const query = args.join(' ');
                    if (!query) return reply(sock, from, msg, `➣ *Example: .${command} Nakupenda*` + commandFooter());
                    ...
                    try {
                        ...
                        let searchData;
                        try {
                            const searchRes = await fetch(`https://api.dreaded.site/api/yts/search?q=${encodeURIComponent(query)}`);
                            searchData = await searchRes.json();
                        } catch (e) {
                            // Fallback Search: itzpire
                            const searchRes = await fetch(`https://itzpire.com/search/youtube?query=${encodeURIComponent(query)}`);
                            const data = await searchRes.json();
                            if (data.status === 'success') {
                                searchData = { success: true, result: data.data };
                            }
                        }
                        
                        if (!searchData || !searchData.success || !searchData.result || searchData.result.length === 0) {
                            return reply(sock, from, msg, `➣ _No results found for: ${query}_` + commandFooter());
                        }
                        ...
                        try {
                            const dlUrl = `https://api.dreaded.site/api/ytdl/${type}?url=${encodeURIComponent(videoUrl)}`;
                            const dlRes = await fetch(dlUrl);
                            const dlData = await dlRes.json();
                            if (dlData.success && dlData.result?.download_url) {
                                downloadLink = dlData.result.download_url;
                            }
                        } catch (e) {}

                        if (!downloadLink) {
                            // Fallback Download: itzpire
                            const dlUrl = `https://itzpire.com/download/youtube?url=${encodeURIComponent(videoUrl)}&type=${type}`;
                            const dlRes = await fetch(dlUrl);
                            const dlData = await dlRes.json();
                            if (dlData.status === 'success') {
                                downloadLink = dlData.data.download || dlData.data.video || dlData.data.audio;
                            }
                        }
                        
                        if (!downloadLink) throw new Error('All download sources failed');
