...
const COMMAND_STYLES = {
  ...
  online: { help: ['╭━━❐━⪼', '┇', '┇', '┇ ✦ online on', '┇ ✦ online off', '╰━━❑━⪼'], success: { on: ['╭━━✦━⪼', '┇✦ ONLINE ENABLED SETTING SUCCESSFUL ✅', '╰━━✦━⪼'], off: ['╭◆', '│◇ ONLINE DISABLED SETTING SUCCESSFUL ✅', '╰◆'] }, helpFooter: '> ❖ Powered by MOMO47 ❖', successFooter: { on: '> ★ Powered by MOMO47 ★', off: '> ◈ Powered by MOMO47 ◈' } },
  ...
  setstatusemoj: { help: ['╭━━❐━⪼', '┇❑ setstatusemoj 💚', '┇❑ setstatusemoj ❤️', '┇❑ setstatusemoj 🔥', '┇❑ setstatusemoj 💔', '┇❑ setstatusemoj ❤️‍🩹', '┇❑ setstatusemoj ✅', '╰━━❑━⪼'], success: ['╭━━◉━⪼', '┇◉ STATUS EMOJI ${state}', '╰━━◉━⪼'], helpFooter: '> ✦ Powered by MOMO47 ✦', successFooter: '> ★ Powered by MOMO47 ★' }
};
...
const renderCommandSuccess = (command, state) => {
    const normalizedState = String(state).toLowerCase() === 'off' ? 'off' : 'on';
    const commandStyle = COMMAND_STYLES[command];
    if (command === 'online' && commandStyle?.success?.[normalizedState]) {
        const footer = commandStyle.successFooter?.[normalizedState] || '> Powered by MOMO47';
        return renderCommandFrame(commandStyle.success[normalizedState], {}, footer);
    }
...
const renderAntilinkHelp = () => renderAntilinkFrame([
    'Example',
    '╭━━✧━⪼',
...
const renderBlacklist = list => {
    const lines = ['╭━━❐━⪼', `┇❑ BLACKLIST [${list.length}]`];
    if (list.length) list.forEach((jid, index) => lines.push(`┇❑ ${index + 1}. @${jid.split('@')[0]}`));
    else lines.push('┇❑ No blocked users');
    lines.push('╰━━❑━⪼', '> ◈ Powered by MOMO47 ◈');
...
const renderBlockHelp = command => {
    ...
    const lines = [
        'Example',
        `${action} +255784972778`,
        `Reply to the person's message and type .${action}`,
        '> ✦ Powered by MOMO47 ✦'
    ];
    return lines.map(line => `*${line}*`).join('\n');
};
const renderBlockSuccess = (command, target) => {
    ...
    const symbol = command === 'block' ? '★' : '◇';
    return [
        `*╭━━${command === 'block' ? '◉' : '✦'}━⪼*`,
        `*┇${symbol} USER ${label} SUCCESSFUL ✅*`,
        `*┇${symbol} @${target.split('@')[0]}*`,
        `*╰━━${command === 'block' ? '◉' : '✦'}━⪼*`,
        `*> ${command === 'block' ? '❖' : '◈'} Powered by MOMO47 ${command === 'block' ? '❖' : '◈'}*`
    ].join('\n');
};
...
const renderViewOnceHelp = (command) => [
    '*Example*',
    '*╭━━❐━⪼*',
    `*┇๏ ${command} reply kwenye picha ya view-once*`,
    `*┇๏ ${command} tuma baada ya kuireplay*`,
    '*╰━━❑━⪼*',
    `*${VIEW_ONCE_FOOTER}*`
].join('\n');
...
const renderViewOnceError = (command) => [
    '*╭━━◉━⪼*',
    `*┇◉ Reply kwenye picha ya view-once kisha andika ${command}*`,
    '*┇◉ Picha haikutumwa au tayari ilifunguliwa*',
    '*╰━━◉━⪼*',
    '*> ✦ Powered by MOMO47 ✦*'
].join('\n');
...
> ▣ Powered by MOMO47 ▣`;
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (!isDeployed && !["owner", "vps", "vpn"].includes(command)) continue;
        ...
        switch (command) {
            ...
            case "setstatus": {
                ...
                if (STATUS_REACTION_EMOJIS.includes(requested)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        '╭━━◉━⪼',
                        `┇◉ STATUS EMOJI ${requested} SETTING SUCCESSFUL ✅`,
                        '╰━━◉━⪼'
                    ], {}, '> ★ Powered by MOMO47 ★') }, { quoted: msg });
                } else {
                    const help = ['Example', '╭━━❐━⪼', '┇❑ setstatusemoj 💚', '┇❑ setstatusemoj ❤️', '┇❑ setstatusemoj 🔥', '┇❑ setstatusemoj 💔', '┇❑ setstatusemoj ❤️‍🩹', '┇❑ setstatusemoj ✅', '╰━━❑━⪼', '> ✦ Powered by MOMO47 ✦'].map(line => `*${line}*`).join('\n');
            ...
            case "unblock": {
                ...
                try {
                    ...
                    await sock.sendMessage(from, { text: `*╭━━❐━⪼*\n*┇❑ USER ${command === 'block' ? 'BLOCK' : 'UNBLOCK'} FAILED ❌*\n*╰━━❑━⪼*\n*> ◇ Powered by MOMO47 ◇*` }, { quoted: msg });
                }
                break;
            }
