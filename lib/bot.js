...
const renderCommandHelp = command => {
    ...
    const inner = texts.map(text => `${prefix}${mapping.inner} ${text.replace(/^[◉★๏❐◈◇❖◆]\s*/u, '')}`);
    return `*Example*\n${renderCommandFrame([frameText(mapping.family, mapping.frame, 'top'), ...inner, frameText(mapping.family, mapping.frame, 'bottom')], {}, '', 'help', mapping.footer)}`;
};
...
const renderCommandSuccess = (command, state) => {
    ...
    if (mapped) {
        ...
        return renderCommandFrame([
            ...
            `${prefix}${mapped.inner} ${title}`,
            frameText(mapped.family, mapped.frame, 'bottom')
        ], {}, '', 'success', mapped.footer);
    }
