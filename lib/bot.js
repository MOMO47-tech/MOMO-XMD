...
const COMMAND_STYLES = {
  ...
  autolikestatus: { help: ['╭◆', '│❖ autolikestatus', '│', '│ ❖ autolikestatus on', '│ ❖ autolikestatus off', '╰◆'], success: ['╭━━๏━⪼', '┇๏ AUTOLIKESTATUS ${state}', '╰━━๏━⪼'], helpFooter: '> ◈ Powered by MOMO47 ◈', successFooter: '> ❑ Powered by MOMO47 ❑' },
...
const enforceAntibug = async (sock, from, msg, body) => {
    ...
    try {
        ...
        if (senderJid && senderJid !== sock.user?.id) {
            ...
            if (typeof sock.reportMessage === 'function') await sock.reportMessage(msg.key).catch(() => {});
        }
        console.warn('[ANTIBUG] removed and blocked suspicious message from', senderJid);
    } catch (error) {
        console.warn('[ANTIBUG] action failed:', error?.message || error);
    }
    return true;
};
