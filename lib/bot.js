...
async function startBot(options = {}) {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const freeCommands = new Set(["menu", "ping", "channel", "group", "owner", "pair", "repo", "tagall"]);
        ...
        switch (command) {
            ...
            case "repo": {
                ...
                const listLines = [
                    ...
                    ...channelLinks.flatMap(link => [`${listFrame.border[1]}${listFrame.symbol} ${link}`, `${listFrame.border[1]}${listFrame.symbol} `]),
            ...
            case "pair": {
                ...
                try {
                    ...
                    const instructions = [
                        `${pairFrame.border[0]}`,
                        `${pairFrame.border[1]}${pairFrame.symbol} LINK DEVICE`,
                        `${pairFrame.border[1]}${pairFrame.symbol} 1. open whatsapp`,
                        `${pairFrame.border[1]}${pairFrame.symbol} 2. open settings > linked devices`,
                        `${pairFrame.border[1]}${pairFrame.symbol} 3. tap link a device`,
                        `${pairFrame.border[1]}${pairFrame.symbol} 4. tap link with phone number instead`,
                        `${pairFrame.border[1]}${pairFrame.symbol} 5. enter the code sent above`,
                        pairFrame.border[2]
                    ];
