...
const synthesizeSpeech = async text => {
    const apiKey = config.openaiApiKey || process.env.OPENAI_API_KEY;
    const apiBase = String(config.openaiApiBase || process.env.OPENAI_API_BASE || '').replace(/\/$/, '');
    if (!apiKey || !apiBase) throw new Error('TTS service is not configured');
...
const hasUnsafeLink = body => {
    const matches = String(body || '').match(/(?:https?:\/\/|www\.|(?:[a-z0-9-]+\.)+(?:com|net|org|xyz|top|info|site|click|link|icu|shop|vip)\b)[^\s]*/gi) || [];
...
const enforceAntispam = async (sock, from, msg, body) => {
    ...
    try {
        ...
        if (normalizedSender && typeof sock.updateBlockStatus === 'function') {
            actions.push(sock.updateBlockStatus(normalizedSender, 'block'));
            if (senderJid && senderJid !== normalizedSender) actions.push(sock.updateBlockStatus(senderJid, 'block'));
        }
        if (typeof sock.reportMessage === 'function') actions.push(sock.reportMessage(msg.key, true));
        await Promise.allSettled(actions);
        if (normalizedSender) blockedUsers.add(normalizedSender);
        saveBlockedUsers();
    } catch (error) {
...
const permissionReply = (kind, message) => {
    ...
    return renderCommandFrame([
        frame.border[0],
        `${frame.border[1]}${frame.symbol} ${String(kind).toUpperCase()} — ${message}`,
        frame.border[2]
    ], {}, frame.footer);
};
...
async function startBot(options = {}) {
            ...
            if (statusCode !== DisconnectReason.loggedOut) {
                ...
                if (!reconnectTimer) {
                    reconnectTimer = setTimeout(() => {
                        reconnectTimer = null;
                        startBot(options).catch(error => console.warn('[BOT] Reconnect failed:', error?.message || error));
                    }, 10000);
                }
            } else {
    ...
    sock.ev.on("messages.delete", async event => {
        if (!runtimeSettings.antidelete) return;
        ...
        for (const key of keys) {
            ...
            try {
                ...
                deletedMessageCache.delete(`${key.remoteJid}:${key.id}`);
            }
        }
    });
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        const freeCommands = new Set(["menu", "ping", "channel", "group", "owner", "pair", "repo", "tagall", "hidetag"]);
        ...
        const ownerCommands = new Set(["online", "antibug", "antidelete", "antispam", "autolikestatus", "autorecording", "autosavestatus", "autotyping", "autoviewonce", "autoviewstatus", "blacklist", "block", "chatbot", "create", "getpp", "mode", "restart", "runtime", "say", "setfont", "setstatusemoj", "setstatus", "tostatus", "unblock", "vv", "vv2", "vps", "vpn"]);
        const senderJid = msg.key.participant || msg.key.remoteJid || "";
        ...
        switch (command) {
            ...
            case "owner": {
                for (const number of ['255765409584', '255760298574']) {
                    await sock.sendMessage(from, {
                        contacts: [{
                            displayName: 'MOMO47',
            ...
            case "tosgroup": {
                ...
                let statusOptions = {};
                if (command === 'tosgroup' && !isGroup) {
                    await sock.sendMessage(from, { text: groupOnlyText() }, { quoted: msg });
                    break;
                }
                if (command === 'tosgroup') {
                    const groupMeta = await getCachedGroupMetadata(sock, from);
                    statusOptions = { statusJidList: (groupMeta?.participants || []).map(item => item.id || item.jid).filter(Boolean) };
                }
                if (!caption && !quoted?.message) {
                    const example = command === 'tostatus'
                        ? ['╭━━❖━⪼', '┇❖ tostatus {caption}', '┇❖ tostatus replay to a message and type tostatus', '╰━━❖━⪼']
                        : ['╭━━❖━⪼', '┇❖ tosgroup [caption]', '┇❖ tosgroup replay to a message and type tosgroup', '╰━━❖━⪼'];
                ...
                try {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        frame.border[0], `${frame.border[1]}${frame.symbol} ${command.toUpperCase()} SUCCESSFUL ✅`, frame.border[2]
            ...
            case "say": {
                ...
                try {
                    const audio = await synthesizeSpeech(caption);
                    await sock.sendMessage(from, { audio, mimetype: 'audio/mpeg', ptt: true }, { quoted: msg });
                } catch (error) {
            ...
            case "listcode": {
                ...
                const requested = args.join(' ').split(/[,+\s]+/).map(value => value.replace(/^\+/, '')).filter(Boolean);
                ...
                try {
                    const metadata = await getCachedGroupMetadata(sock, from);
                    const members = (metadata?.participants || []).map(item => item.id || item.jid).filter(Boolean);
                    const sections = [];
                    for (const code of codes) {
                        const matches = members.filter(jid => String(jid).split('@')[0].split(':')[0].replace(/\D/g, '').startsWith(code));
                        sections.push(`${COUNTRY_CODES[code]} (${code}) — ${matches.length}`);
                        sections.push(...(matches.length ? matches.map(jid => `+${String(jid).split('@')[0].split(':')[0].replace(/\D/g, '')}`) : ['No numbers found']));
                        sections.push('');
                    }
                    const frame = uniqueCommandFrame(`listcode:${codes.join(',')}`, 'LIST');
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        frame.border[0],
                        `${frame.border[1]}${frame.symbol} ${requested.includes('all') ? 'LIST ALL COUNTRIES' : `LIST NUMBER OF ${COUNTRY_CODES[codes[0]]} IN THE GROUP`}`,
                        ...sections.map(line => line ? `${frame.border[1]}${frame.symbol} ${line}` : ''),
                        frame.border[2]
            ...
            case "open": {
                ...
                await sock.sendMessage(from, { text: renderCommandFrame([
                    groupFrame.border[0],
                    `${groupFrame.border[1]}${groupFrame.symbol} GROUP ${command.toUpperCase()} SUCCESSFUL ✅`,
                    groupFrame.border[2]
            ...
            case "antibot": {
                ...
                if (["on", "off"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        ...
                        `${antibotFrame.border[1]}${antibotFrame.symbol} ANTIBOT ${opt.toUpperCase()} SETTING SUCCESSFUL ✅`,
            ...
            case "antigif": {
                ...
                if (["on", "off"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        ...
                        `${filterFrame.border[1]}${filterFrame.symbol} ${command.toUpperCase()} ${opt.toUpperCase()} SETTING SUCCESSFUL ✅`,
            ...
            case "announcements": {
                ...
                if (["on", "off"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        ...
                        `${announcementFrame.border[1]}${announcementFrame.symbol} ANNOUNCEMENTS ${opt.toUpperCase()} SETTING SUCCESSFUL ✅`,
            ...
            case "antivirus": {
                ...
                if (["on", "off"].includes(opt)) {
                    ...
                    await sock.sendMessage(from, { text: renderCommandFrame([
                        ...
                        `${antivirusFrame.border[1]}${antivirusFrame.symbol} ANTIVIRUS ${opt.toUpperCase()} SETTING SUCCESSFUL ✅`,
