...
async function startBot() {
    ...
    sock.ev.on("messages.upsert", async (m) => {
        ...
        if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT_ERROR || 
            msg.message?.protocolMessage?.type === proto.Message.ProtocolMessage.Type.EPHEMERAL_SETTING) {
            console.log('[SESSION] ⚠️ Decryption error detected. Clearing problematic session keys...');
            try {
                // Clear session store to force re-handshake
