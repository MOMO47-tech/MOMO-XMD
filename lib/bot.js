...
const downloadQuotedMedia = async (msg) => {
    ...
    if (!(await isDecodableMediaBuffer(buffer, mediaType))) throw new Error('quoted media download returned invalid data; reply to the original sticker/video');
...
async function startBot(options = {}) {
    ...
    sock.ev.on('group-participants.update', async (update) => {
        ...
        const author = update?.author || update?.participant || null;
        if ((settings.antileft || settings.alive) && ['remove', 'leave'].includes(action) && typeof sock.groupParticipantsUpdate === 'function') {
            for (const participant of participants) {
                ...
                const selfLeft = action === 'leave' || (userNumber && authorNumber && userNumber === authorNumber);
            ...
            case "tosgroup": {
                ...
                try {
                    ...
                    if (groupQuoted?.message) {
                        ...
                        const media = quotedContent[quotedType] || {};
                        if (['imageMessage', 'videoMessage', 'audioMessage', 'documentMessage', 'stickerMessage'].includes(quotedType)) {
                            const source = { key: { remoteJid: from, id: groupQuoted.stanzaId || `quoted-${Date.now()}`, participant: groupQuoted.participant }, message: quotedContent };
                            const buffer = await downloadMediaMessage(source, 'buffer', {}, { logger: pino({ level: 'silent' }), reuploadRequest: sock.updateMediaMessage });
